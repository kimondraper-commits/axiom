...
function SparklinePath({ data, color }: { data: number[]; color: string }) {
  ...
  const gradId = `sfill-${color.replace(/[^a-z0-9]/gi, "")}`;
...
export function StatCard({ label, value, highlight, alert, index = 0, trend }: StatCardProps) {
  ...
  const sparkColor = alert ? "#DC2626" : highlight ? "#C9A84C" : "#34D399";
  const trendUp = typeof sparkData !== "undefined" && sparkData[sparkData.length - 1] >= sparkData[0];
